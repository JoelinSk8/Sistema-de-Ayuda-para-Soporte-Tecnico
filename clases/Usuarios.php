...
class Usuarios extends Conexion
{
    public function loginUsuario($usuario, $password)
    {
        ...
        //para ver activar o no
        /* $activo = mysqli_fetch_array($respuesta);
    ...
    public function agregaNuevoUsuario($datos)
    {
        ...
        if ($idPersona > 0) {
            $sql = "INSERT INTO t_usuarios (id_rol,
                                            id_persona,usuario,password,ubicacion)
            VALUES(?, ?, ?, ?, ?)";
    ...
    public function cambioEstatusUsuario($idUsuario, $estatus)
    {
        ...
        $sql = "UPDATE t_usuarios SET activo=? 
                                    WHERE id_usuario = ?";
        $query = $conexion->prepare($sql);
        $query->bind_param('ii', $idUsuario, $estatus);
        $respuesta = $query->execute();
        $query->close();
